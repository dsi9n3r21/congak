...
import { restartAdventure } from "@/lib/actions/missions";

export function ReplayAdventureButton({ mode, lang }: { mode: MissionMode; lang: Lang }) {
  ...
  return (
    <div>
      <button
        ...
        onClick={() =>
          startTransition(async () => {
            setError(false);
            const result = await restartAdventure(mode).catch(() => ({ ok: false }));
            if (!result.ok) {
              setError(true);
              return;
            }
      ...
      >
        <Bi text={{ ms: "Main Semula Peta", en: "Play Adventure Again" }} lang={lang} />
      </button>
      {error && (
