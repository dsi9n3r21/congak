...
// scales similarly to a real paper's item count, not just proportionally
// to the shorter practice durations.
const DURATION_OPTIONS = [
  { minutes: 5, questionCount: 6 },
  { minutes: 10, questionCount: 10 },
  { minutes: 15, questionCount: 16 },
  { minutes: 90, questionCount: 40 },
];
...
export function ExamFlow({ lang }: { lang: Lang }) {
    ...
    return (
      <div className="rounded-kite bg-white p-5 shadow-card">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink/50">
          <Bi text={UI.chooseTopics} lang={lang} />
        </p>
          ...
          {Object.values(TOPICS).map((t) => (
            <label key={t.id} className="flex items-center gap-2 rounded-kite border-2 border-ink/10 px-3 py-2.5">
              <input
                ...
                className="h-4 w-4"
              />
              <span className="text-sm text-ink"><Bi text={t.title} lang={lang} /></span>
            </label>
          ))}
        </div>

        <p className="mb-2 mt-5 text-xs font-semibold uppercase tracking-wide text-ink/50">
          <Bi text={UI.chooseDuration} lang={lang} />
        </p>
        <div className="grid grid-cols-2 gap-2">
          {DURATION_OPTIONS.map((opt) => (
            <button
              ...
              className={`rounded-kite border-2 py-2.5 text-sm font-semibold min-h-[44px] ${
                duration.minutes === opt.minutes ? "border-kuning bg-kuning-light" : "border-ink/10 text-ink/60"
              }`}
            >
              {opt.minutes >= 60 ? `${opt.minutes / 60} j` : `${opt.minutes} min`}
              {opt.minutes === 90 && <span className="ml-1 text-[10px] font-normal">(spt sekolah)</span>}
            </button>
          ))}
        </div>
        ...
        >
          <Bi text={UI.startExam} lang={lang} /> →
        </button>
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        <div className="rounded-kite bg-white p-5 shadow-card">
          ...
          >
            {submitting ? "..." : index === questions.length - 1 ? <Bi text={UI.submitQuiz} lang={lang} /> : <><Bi text={UI.nextArrow} lang={lang} /> →</>}
          </button>
        </div>
      </div>
    );
    ...
    <div>
      <div className="rounded-kite bg-white p-6 text-center shadow-card">
        ...
        <p className="mt-2 font-display text-2xl font-bold text-ink">
          {r.score}/{r.total} <Bi text={UI.quizScore} lang={lang} />
        </p>
      ...
      {r.strengths.length > 0 && (
        <div className="mt-4 rounded-kite bg-pandan-light p-4">
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-pandan-dark">
            <Bi text={UI.strengths} lang={lang} />
          </p>
          <ul className="space-y-1 text-sm text-ink">
            {r.strengths.map((id) => (
              <li key={id}>✓ <Bi text={TOPICS[id]?.title} lang={lang} /></li>
            ))}
          </ul>
        </div>
      )}
      ...
      {r.weaknesses.length > 0 && (
        <div className="mt-4 rounded-kite bg-saga-light/40 p-4">
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-saga-dark">
            <Bi text={UI.needsImprovement} lang={lang} />
          </p>
          <ul className="space-y-1 text-sm text-ink">
            {r.weaknesses.map((id) => (
              <li key={id}>△ <Bi text={TOPICS[id]?.title} lang={lang} /></li>
      ...
      <div className="mt-4 rounded-kite bg-biru-light/40 p-4">
        ...
        <div className="space-y-2">
          {r.recommendedPath.map((id) => (
            ...
            >
              <Bi text={TOPICS[id]?.title} lang={lang} /> →
            </Link>
          ))}
        </div>
      </div>
